...
export function Calculator() {
  const { state, inputDigit, chooseOperator, evaluateExpression, clearAll } =
    useCalculator();
  ...
  return (
    <div className="calc-root">
      ...
      <div className="calc-grid">
        ...
        <button className="op" onClick={() => chooseOperator("/")}>
          /
        </button>
        <button className="op" onClick={() => chooseOperator("*")}>
          *
        ...
        {[7, 8, 9, 4, 5, 6, 1, 2, 3].map((n) => (
          <button key={n} onClick={() => inputDigit(n)}>
            {n}
          </button>
        ))}
        ...
        <button className="eq" onClick={() => evaluateExpression()}>
          =
        </button>
      </div>
    </div>
